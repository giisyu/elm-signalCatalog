...
Elm.SignalCatalog = Elm.SignalCatalog || {};
Elm.SignalCatalog.make = function (_elm) {
   "use strict";
   _elm.SignalCatalog = _elm.SignalCatalog || {};
   if (_elm.SignalCatalog.values)
   ...
   $SignalExtraDemo = Elm.SignalExtraDemo.make(_elm),
   $Task = Elm.Task.make(_elm),
   $Text = Elm.Text.make(_elm),
   $Time = Elm.Time.make(_elm),
   ...
   var set = Elm.Native.Task.make(_elm).performSignal("set",
   A2($Signal._op["<~"],
   function (x) {
      return function () {
         switch (x.ctor)
         {case "Just":
            return $History.setPath(x._0);
            case "Nothing":
            return $Task.fail("");}
         _U.badCase($moduleName,
   ...
   hashClick.signal));
   var makebutton = function (_v9) {
      return function () {
         switch (_v9.ctor)
         {case "_Tuple2":
            return function () {
                 var defaultText = function (str) {
                    return $Graphics$Element.justified($Text.style($Text.defaultStyle)($Text.fromString(str)));
                 };
                 var ele = A4($Graphics$Element.container,
                 140,
                 20,
   ...
   var sendLine1 = $Signal.mailbox(0);
   var sendCount1 = Elm.Native.Task.make(_elm).performSignal("sendCount1",
   A2($Signal.map,
   function (x) {
      return function () {
         switch (x.ctor)
   ...
   "sendLine1.signal",
   "sendLine2.signal",
   $SignalDemo.clickCount,
   sendLine1.signal,
   sendLine2.signal);
   var taskSignalDemo = A2($View.signalFlow,
...
Elm.SignalDemo = Elm.SignalDemo || {};
Elm.SignalDemo.make = function (_elm) {
   "use strict";
   _elm.SignalDemo = _elm.SignalDemo || {};
   if (_elm.SignalDemo.values)
   return _elm.SignalDemo.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "SignalDemo",
   $Basics = Elm.Basics.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   ...
   $Time = Elm.Time.make(_elm),
   $View = Elm.View.make(_elm);
   var fpsTest = $Time.fps(1);
   var mergeTest = A2($Signal.merge,
   $Mouse.isDown,
   $Keyboard.enter);
   var mouseIsDown = $Mouse.isDown;
   var mergeDemo = A7($View.line3,
   ...
   $Keyboard.enter,
   mergeTest);
   var countTime = A3($Signal.foldp,
   F2(function (x,y) {
      return y + 1;
   }),
   0,
   $Time.every($Time.second));
   var trueClick = A2($Signal.map,
   $Basics.always(true),
   $Mouse.clicks);
   var sampleOnSignal = A2($Signal.sampleOn,
   trueClick,
   fpsTest);
   var sampleOnDemo = A7($View.line3,
   ...
   fpsTest,
   sampleOnSignal);
   var mouseClick = $Mouse.clicks;
   var mapDemo = A2($View.signalFlow,
   $Graphics$Element.down,
   _L.fromArray([$View.functionName("map : (a -> result) -> Signal a -> Signal result")
                ,$View.description("Apply a function to a signal.")
                ,A2($View.basicCatalog,
                mouseClick,
                "Mouse.clicks")
                ,A3($View.customCatalog,
                trueClick,
                function (x) {
                   return _U.eq(x,
                   "True") ? " T" : x;
   ...
   0,
   mouseClick);
   var multiplesOf3 = A3($Signal.filter,
   function (x) {
   ...
   clickCount,
   multiplesOf3);
   var foldpDemo = A5($View.line2,
   "foldp : (a -> state -> state) -> state -> Signal a -> Signal state",
   "Mouse.clicks",
   "clickCount = Signal.foldp (\\x y -> y + 1 ) 0 mouseClick",
   mouseClick,
   clickCount);
   var sinceTest = A2($Time.since,
   1 * $Time.second,
   mouseClick);
   var droTest = $Signal.dropRepeats(sinceTest);
   var dropRepeatsDemo = A5($View.line2,
   "dropRepeats : Signal a -> Signal a",
   "Time.since (1 * second) Mouse.clicks",
   "Signal.dropRepeats sinceTest",
   sinceTest,
   droTest);
   var signalDemo = A2($View.signalFlow,
   $Graphics$Element.down,
   _L.fromArray([$View.title("Signal")
                ,$View.demoList(_L.fromArray([mapDemo
                                             ,map2Demo
                                             ,foldpDemo
                                             ,mergeDemo
                                             ,filterDemo
                                             ,dropRepeatsDemo
                                             ,sampleOnDemo]))]));
   _elm.SignalDemo.values = {_op: _op
                            ,signalDemo: signalDemo
...
Elm.SignalExtraDemo = Elm.SignalExtraDemo || {};
Elm.SignalExtraDemo.make = function (_elm) {
   "use strict";
   _elm.SignalExtraDemo = _elm.SignalExtraDemo || {};
   if (_elm.SignalExtraDemo.values)
   return _elm.SignalExtraDemo.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "SignalExtraDemo",
   $Basics = Elm.Basics.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $Keyboard = Elm.Keyboard.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Time = Elm.Time.make(_elm),
   $View = Elm.View.make(_elm);
   var tickcount = A2($Signal$Extra._op["~>"],
   $Signal$Time.relativeTime($Time.every($Time.second)),
   function ($) {
      return $Basics.round($Time.inSeconds($));
   });
   var relativeTest = $Signal$Time.relativeTime($Time.every($Time.second));
   var relativeTimeDemo = A5($View.line2,
   "relativeTime : Signal Time -> Signal Time",
   "relativeTime (Time.every second)",
   "relativeTime (Time.every second) ~> Time.inSeconds >> round",
   relativeTest,
   tickcount);
   var start = $Signal$Time.startTime;
   var startTimeDemo = A3($View.line1,
   "startTime : Signal Time",
   "startTime",
   start);
   var tooltip = A2($Signal.merge,
   A2($Signal._op["<~"],
   $Basics.always(false),
   $Mouse.position),
   A2($Signal._op["<~"],
   $Basics.always(true),
   $Signal$Time.settledAfter(500 * $Time.millisecond)($Mouse.position)));
   var mousePosition = $Mouse.position;
   var after = A2($Signal$Time.settledAfter,
   500 * $Time.millisecond,
   mousePosition);
   var settledAfterDemo = A7($View.line3,
   ...
   after,
   tooltip);
   var noDoubleClicks = A2($Signal$Time.dropWithin,
   300 * $Time.millisecond,
   $Mouse.clicks);
   var dropWithinDemo = A5($View.line2,
   "dropWithin : Time -> Signal a -> Signal a",
   "Mouse.clicks",
   "noDoubleClicks = ExTime.dropWithin (300 * millisecond) Mouse.clicks",
   $SignalDemo.mouseClick,
   noDoubleClicks);
   var throttledMouseClicks = A2($Signal$Time.limitRate,
   2 * $Time.second,
   $Mouse.clicks);
   var limitRateDemo = A5($View.line2,
   "limitRate : Time -> Signal a -> Signal a",
   "Mouse.clicks",
   "throttledMouseClicks = limitRate (2 * second) Mouse.clicks",
   $SignalDemo.mouseClick,
   throttledMouseClicks);
   var keepWhenITest = A2($Signal$Extra.keepWhenI,
   $SignalDemo.mouseIsDown,
   $Time.fps(1));
   var keepWhenIDemo = A7($View.line3,
   ...
   $SignalDemo.fpsTest,
   keepWhenITest);
   var keepThenTest = A3($Signal$Extra.keepThen,
   $SignalDemo.mouseIsDown,
   0,
   $Time.fps(1));
   var keepThenDemo = A7($View.line3,
   ...
   $SignalDemo.fpsTest,
   keepThenTest);
   var sampleWhenTest = A3($Signal$Extra.sampleWhen,
   $SignalDemo.mouseIsDown,
   0,
   $Time.fps(1));
   var sampleWhenDemo = A7($View.line3,
   ...
   $SignalDemo.fpsTest,
   sampleWhenTest);
   var keepWhenTest = A3($Signal$Extra.keepWhen,
   $SignalDemo.mouseIsDown,
   0,
   $Time.fps(1));
   var keepWhenDemo = A7($View.line3,
   ...
   $Time.fps(1),
   keepWhenTest);
   var switchSampleTest = A3($Signal$Extra.switchSample,
   $SignalDemo.mouseIsDown,
   $Time.fps(2),
   $Time.every($Time.second));
   var fps2 = $Time.fps(2);
   var switchSampleDemo = A7($View.line3,
   ...
   $Time.every($Time.second),
   switchSampleTest);
   var switchWhenTest = A3($Signal$Extra.switchWhen,
   $SignalDemo.mouseIsDown,
   $Time.fps(2),
   $Time.every($Time.second));
   var switchWhenDemo = A7($View.line3,
   ...
   fps2,
   switchWhenTest);
   var derayRTest = A2($Signal$Extra.delayRound,
   0,
   $SignalDemo.clickCount);
   var delayRoundDemo = A5($View.line2,
   "delayRound : b -> Signal b -> Signal b",
   "clickCount",
   "Extra.delayRound 0 clickCount",
   $SignalDemo.clickCount,
   derayRTest);
   var count = function (signal) {
      return A3($Signal.foldp,
      F2(function (s,a) {
         return a + 1;
      }),
   ...
   var run = A2($Signal$Extra.runBuffer,
   5,
   count($Time.every($Time.second)));
   var runlist = A2($Signal._op["<~"],
   F2(function (x,y) {
      return _U.eq(x,y);
   })(_L.fromArray([1,2,3,4,5])),
   A2($Signal$Extra.runBuffer,
   5,
   count($Time.every($Time.second))));
   var runBufferDemo = A5($View.line2,
   "runBuffer : Int -> Signal a -> Signal (List a)",
   "runBuffer 5 (count (Time.every second))",
   "((==) [1,2,3,4,5]) <~ runBuffer 5 (count (Time.every second))",
   run,
   runlist);
   var zipTest = A2($Signal$Extra.zip,
   $Mouse.x,
   $Mouse.y);
   var zipDemo = A3($View.line1,
   "zip : Signal a -> Signal b -> Signal (a,b)",
   "Extra.zip Mouse.x Mouse.y ",
   zipTest);
   var sampleTest = A3($Signal$Stream.sample,
   F2(function (v0,v1) {
      return {ctor: "_Tuple2"
             ,_0: v0
             ,_1: v1};
   ...
   $Keyboard.arrows,
   $Signal$Stream.fromSignal($Time.fps(1)));
   var sampleDemo = A3($View.line1,
   "sample : (a -> b -> c) -> Signal a -> Stream b -> Stream c",
   "Stream.sample (,) Keyboard.arrows (Stream.fromSignal <| fps 1)",
   sampleTest);
   var enter = A2($Signal$Discrete.whenChangeTo,
   true,
   $Keyboard.enter);
   var toggleOnEnter = A3($Signal$Discrete.folde,
   $Basics.not,
   false,
   enter);
   var foldeDemo = A5($View.line2,
   ...
   enter,
   toggleOnEnter);
   var whenChangeToTest = A2($Signal$Discrete.whenChangeTo,
   true,
   $Keyboard.enter);
   var whenChangeToDemo = A5($View.line2,
   "whenChangeTo: a -> Signal a -> EventSource",
   "Keyboard.enter",
   "Discrete.whenChangeTo True Keyboard.enter",
   $Keyboard.enter,
   whenChangeToTest);
   var sinceTest2 = A2($Time.since,
   500 * $Time.millisecond,
   $SignalDemo.mouseClick);
   var whenChangeTest = $Signal$Discrete.whenChange(sinceTest2);
   var whenChangeDemo = A5($View.line2,
   "whenChange : Signal a -> EventSource",
   "sinceTest2 = Time.since (500 * millisecond) mouseClick",
   "Discrete.whenChange sinceTest2",
   sinceTest2,
   whenChangeTest);
   var equalTest = A2($Signal$Discrete.whenEqual,
   true,
   $Mouse.isDown);
   var whenEqualDemo = A2($View.signalFlow,
   $Graphics$Element.down,
   _L.fromArray([$View.functionName("whenEqual : a -> Signal a -> EventSource")
                ,A3($View.customCatalog,
                $SignalDemo.mouseIsDown,
                function (x) {
                   return _U.eq(x,
                ...
                ,A2($View.basicCatalog,
                equalTest,
                "Discrete.whenEqual True Mouse.isDown")]));
   var esTest = $Signal$Discrete.es($Time.every($Time.second));
   var esDemo = A2($View.signalFlow,
   $Graphics$Element.down,
   _L.fromArray([$View.functionName("es : Signal a -> EventSource")
                ,A3($View.customCatalog,
                $Time.every($Time.second),
                function (x) {
                   return A2($String.left,3,x);
                },
                "every second")
                ,A2($View.basicCatalog,
                esTest,
                "Discrete.es (every second)")]));
   var signalExtraDemo = A2($View.signalFlow,
   $Graphics$Element.down,
   _L.fromArray([$View.title("Signal.Discrete")
                ,$View.functionName("type alias EventSource = Signal ()")
                ,$View.demoList(_L.fromArray([esDemo
                                             ,whenEqualDemo
                                             ,whenChangeDemo
                                             ,whenChangeToDemo
                                             ,foldeDemo]))
                ,$View.title("Signal.Extra")
                ,$View.demoList(_L.fromArray([zipDemo
                                             ,runBufferDemo
                                             ,delayRoundDemo
                                             ,switchWhenDemo
                                             ,switchSampleDemo
                                             ...
                                             ,keepThenDemo
                                             ,keepWhenIDemo]))
                ,$View.title("Signal.Stream")
                ,$View.demoList(_L.fromArray([sampleDemo]))
                ,$View.title("Signal.Time")
                ,$View.demoList(_L.fromArray([limitRateDemo
                                             ,dropWithinDemo
                                             ,settledAfterDemo
                                             ,startTimeDemo
                                             ,relativeTimeDemo]))]));
   _elm.SignalExtraDemo.values = {_op: _op
                                 ,signalExtraDemo: signalExtraDemo};
   return _elm.SignalExtraDemo.values;
};
...
Elm.TimeDemo = Elm.TimeDemo || {};
Elm.TimeDemo.make = function (_elm) {
   "use strict";
   _elm.TimeDemo = _elm.TimeDemo || {};
   if (_elm.TimeDemo.values)
   return _elm.TimeDemo.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   1 * $Time.second,
   $SignalDemo.mouseClick);
   var sinceDemo = A5($View.line2,
   "since : Time -> Signal a -> Signal Bool",
   "mouseClick = Mouse.clicks",
   "Time.since (1 * second) mouseClick",
   $SignalDemo.mouseClick,
   sinceTest);
   var delayTest = A2($Time.delay,
   500 * $Time.millisecond,
   $SignalDemo.mouseClick);
   var delayDemo = A5($View.line2,
   "delay : Time -> Signal a -> Signal a",
   "mouseClick = Mouse.clicks",
   "delay (500 * millisecond) mouseClick",
   $SignalDemo.mouseClick,
   delayTest);
   var timstam = $Time.timestamp($SignalDemo.mouseClick);
   var timestampDemo = A5($View.line2,
   "timestamp : Signal a -> Signal (Time, a)",
   "mouseClick = Mouse.clicks",
   "Time.timestamp mouseClick",
   $SignalDemo.mouseClick,
   timstam);
   var everyTest = $Time.every(1 * $Time.second);
   var everyDemo = A3($View.line1,
   "every : Time -> Signal Time",
   "Time.every (1 * second)",
   everyTest);
   var fpsWhenTest = A2($Time.fpsWhen,
   5,
   $SignalDemo.mouseIsDown);
   var fpsWhenDemo = A5($View.line2,
   "fpsWhen : number -> Signal Bool -> Signal Time",
   "mouseIsDown = Mouse.isDown",
   "Time.fpsWhen 5 mouseIsDown",
   $SignalDemo.mouseIsDown,
   fpsWhenTest);
   var fpsDemo = A3($View.line1,
   "fps : number -> Signal Time",
   "fps 1",
   $SignalDemo.fpsTest);
   var timeDemo = A2($View.signalFlow,
   $Graphics$Element.down,
   _L.fromArray([$View.title("Time")
                ,$View.demoList(_L.fromArray([fpsDemo
                                             ,fpsWhenDemo
                                             ,everyDemo
                                             ,timestampDemo
                                             ,delayDemo
                                             ,sinceDemo]))]));
   _elm.TimeDemo.values = {_op: _op
                          ,timeDemo: timeDemo};
   return _elm.TimeDemo.values;
};
...
Elm.View.make = function (_elm) {
   ...
   $Text = Elm.Text.make(_elm),
   $Time = Elm.Time.make(_elm);
   var descriptionSryle = {_: {}
                          ,bold: false
                          ,color: $Color.black
                          ,height: $Maybe.Just(15)
                          ,italic: false
                          ,line: $Maybe.Nothing
                          ,typeface: _L.fromArray(["monospace"])};
   var fucnNameStyle = {_: {}
                       ,bold: true
                       ,color: A3($Color.rgb,255,85,85)
                       ,height: $Maybe.Just(15)
                       ,italic: false
                       ,line: $Maybe.Nothing
                       ,typeface: _L.fromArray(["monospace"])};
   var titleStyle = {_: {}
                    ,bold: true
                    ,color: $Color.black
                    ,height: $Maybe.Just(25)
                    ,italic: false
                    ,line: $Maybe.Nothing
                    ,typeface: _L.fromArray(["monospace"])};
   var code = function (codeStr) {
      return function () {
         var format = function (string) {
            return $Graphics$Element.color(A3($Color.rgb,
            255,
            255,
            255))($Graphics$Element.justified($Text.monospace($Text.fromString(string))));
         };
         return $Signal.constant(A2($Graphics$Element.beside,
   ...
   var timeLineView = function (lineStr) {
      return function () {
         var format = function (string) {
            return $Graphics$Element.justified($Text.monospace($Text.fromString(string)));
         };
         return format(lineStr);
      }();
   };
   var typeEleFormat = function (str) {
      return $Graphics$Element.justified($Text.monospace($Text.fromString(A3($String.padLeft,
      35,
      _U.chr(" "),
      str))));
   };
   var toTypeElement = function (signal) {
      return A2($Signal.map,
      function (x) {
         return typeEleFormat($Basics.toString(x));
      },
      signal);
   };
   var signalSpace = F2(function (x,
   y) {
      return $Signal.constant(A2($Graphics$Element.spacer,
      x,
      y));
   });
   var lineSpace = A2(signalSpace,
   5,
   24);
   var strToElement = F2(function (stringStyle,
   str) {
      return $Graphics$Element.justified(A2($Text.style,
      stringStyle,
      $Text.fromString(str)));
   });
   var box = F4(function (upSpace,
   leftSpace,
   downSpace,
   strEle) {
   ...
   var toSignalElement = F5(function (upSpace,
   leftSpace,
   downSpace,
   strStyle,
   str) {
      return $Signal.constant(A4(box,
      upSpace,
      leftSpace,
      downSpace,
      A2(strToElement,strStyle,str)));
   });
   var title = function (str) {
      return A5(toSignalElement,
      7,
      20,
      8,
      titleStyle,
   ...
   var lineString = "------------------------------------------------------";
   var signalFlow = F2(function (direction,
   list) {
      return A2($Signal$Extra.mapMany,
      $Graphics$Element.flow(direction),
      list);
   });
   var demoList = function (list) {
      return signalFlow($Graphics$Element.down)(A2($Basics._op["++"],
      A2($List.intersperse,
      lineSpace,
      list),
      _L.fromArray([lineSpace])));
   };
   var timefps = $Time.fps(15);
   var Action = function (a) {
      return {ctor: "Action"
             ,_0: a};
   };
   var TimeSignal = {ctor: "TimeSignal"};
   var timeLineUpdate = F3(function (lineString,
   f,
   signal) {
      return function () {
         var update = F2(function (signal,
         state) {
            return function () {
               switch (signal.ctor)
               {case "Action":
                  return function () {
                       var str = f($Basics.toString(signal._0));
                       return $String.dropLeft($String.length(str))(A2($String.append,
                       state,
                       str));
                    }();
                  case "TimeSignal":
                  return $String.dropLeft(1)(A2($String.append,
                    state,
                    "-"));}
               _U.badCase($moduleName,
         ...
         var addTimeSignal = function (signal) {
            return A2($Signal.merge,
            A2($Signal._op["<~"],
            Action,
            signal),
            A2($Signal._op["<~"],
            $Basics.always(TimeSignal),
            timefps));
         };
         var action = addTimeSignal(signal);
         return A3($Signal.foldp,
         update,
         lineString,
         action);
   ...
   var customCatalog = F3(function (signal,
   f,
   codeStr) {
      return A2(signalFlow,
      $Graphics$Element.right,
      _L.fromArray([toTypeElement(signal)
                   ,A2(signalSpace,5,10)
                   ,A2(toTimeLine,f,signal)
                   ,code(codeStr)]));
   });
   var basicCatalog = F2(function (signal,
   str) {
      return A3(customCatalog,
      signal,
      $Basics.identity,
      str);
   });
   var line1 = F3(function (funcName,
   code,
   signal) {
      return A2(signalFlow,
      $Graphics$Element.down,
      _L.fromArray([functionName(funcName)
                   ,A2(basicCatalog,
                   signal,
                   code)]));
   });
   var line2 = F5(function (funcName,
   code1,
   code2,
   signal1,
   signal2) {
      ...
      $Graphics$Element.down,
      _L.fromArray([functionName(funcName)
                   ,A2(basicCatalog,signal1,code1)
                   ,A2(basicCatalog,
                   signal2,
                   code2)]));
   });
   var line3 = F7(function (funcName,
   code1,
   code2,
   code3,
   signal1,
   ...
   signal3) {
      ...
      _L.fromArray([functionName(funcName)
                   ,A2(basicCatalog,signal1,code1)
                   ,A2(basicCatalog,signal2,code2)
                   ,A2(basicCatalog,
                   signal3,
                   code3)]));
   });
   var line3$ = F8(function (funcName,
   refeStr,
   code1,
   code2,
   code3,
